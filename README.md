...
- `--hidden`: hidden size
- `--vision`: vision radius for partial observations
...
## Environment details
- 2D square workspace, agents spawn/goals in [-0.3, 0.3]^2, discrete moves {stay, up, down, left, right} with step size 0.3 (defaults).
- Observations per agent: own position (2), shared goal (2 by default), relative positions of other agents within vision radius (masked to zero otherwise).
...
- Episode ends on success or when `max_steps` reached (default 20). Defaults favor learning (shared goal, larger success bonus, stronger progress shaping).
