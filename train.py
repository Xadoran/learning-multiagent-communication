...
def train_policy(
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        print(
            f"Resumed from episode {start_episode}, {len(episode_returns)} episodes in history"
        )
    ...
    for ep in range(start_episode, episodes):
        ...
        for i in range(batch_size):
            policy_loss = (
                -(batch_logprob_totals[i] * batch_advantages[i]).sum()
                - entropy_beta * batch_entropies[i].mean()
            )
        ...
        if (ep + 1) % max(1, 100 // batch_size) == 0:
            ...
            print(
                f"Episode {ep+1}/{episodes} (total: {total_episodes}) | avg return (last 100): {avg_last:.3f} | batch_size: {batch_size}"
            )
...
def train_lever_supervised(
    ...
    for ep in range(start_episode, episodes):
        ...
        loss = (
            ce_loss(logits.view(-1, policy.action_dim), target_t.view(-1))
        ...
        if (ep + 1) % 100 == 0:
            ...
            print(
                f"[Lever supervised] Episode {ep+1}/{episodes} | avg return (last 100): {avg_last:.3f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--lever_supervised",
        action="store_true",
        help="Use supervised lever policy instead of RL",
    )
    parser.add_argument(
        "--lever_identity_encoder",
        action="store_true",
        help="Use identity encoder for lever game (hidden_dim=obs_dim)",
    )
    parser.add_argument(
        "--lever_batch",
        type=int,
        default=256,
        help="Batch size for supervised lever training",
    )
    parser.add_argument(
        "--lever_rank_features",
        action="store_true",
        help="Add prefix-sum rank features for lever game (requires identity encoder)",
    )
    ...
    parser.add_argument(
        "--comm_mlp",
        action="store_true",
        help="Use 2-layer MLP for comm blocks (more expressive)",
    )
    parser.add_argument(
        "--comm_mlp_hidden",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable encoder tanh activation (useful for lever)",
    )
    parser.add_argument(
        "--no_comm_activation",
        action="store_true",
        help="Disable comm layer tanh activation (useful for lever)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Batch size for RL training (episodes per update)",
    )
    parser.add_argument(
        "--resume_checkpoint",
        type=str,
